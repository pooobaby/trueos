...
int32_t pcb_fd_install(int32_t globa_fd_idx) {
    ...
    uint8_t local_fd_idx = 3;   // 从文件描述符 3 开始
...
// file_open 把 inode 元信息载入了内存, 但没把文件数据载入内存；
// 分配 fd 只是把"已打开文件"登记到进程的文件描述符表里, 让后续 read/write 能通过 fd 找到对应的 inode。
...
int32_t file_read(struct file* file, void* buf, uint32_t count) {
    ...
    // 用到的块地址已经收集到 all_blocks中, 下面开始读数据
