...
static bool mount_partition(struct list_elem* pelem, int arg) {
    ...
    if (!strcmp(part->name, part_name)) {
        ...
        // printk("mount %s done!\n", part->name);
        // printk("cur_part's name: %s\n", cur_part->name);
        // printk(" - disk: %s\n", cur_part->my_disk->name);
        // printk(" - start_lba: 0x%x\n", cur_part->start_lba);
        // printk(" - block_bitmap start: 0x%x, %d\n", cur_part->sb->block_bitmap_lba * 512, cur_part->sb->block_bitmap_lba * 512);
        // printk(" - inode_bitmap start: 0x%x, %d\n", cur_part->sb->inode_bitmap_lba * 512, cur_part->sb->inode_bitmap_lba * 512);
        // printk(" - inode_table start: 0x%x, %d\n", cur_part->sb->inode_table_lba * 512, cur_part->sb->inode_table_lba * 512);
        // printk(" - root dir start: 0x%x, %d\n", cur_part->sb->data_start_lba * 512, cur_part->sb->data_start_lba * 512);
        
        // 此处返回 true 是是为了迎合主调函数 list_traversal 的实现, 与函数本身功能无关。
...
static void partition_format(struct partition* part) {
    ...
    // 2 将 block_bitmap 块位图初始化并写入 sb.block_bitmap_lba
    // 初始化块位图 block_bitmap
    ...
    // 2-1 先将位图最后一字节到其所在的扇区的结束全置为 1, 即超出实际块数的部分直接置为已占用
    ...
    buf[0] |= 0x1;      // 第 0 个inode 分给了根目录
    /* 由于 inode_table 中共 4096 个inode, 位图 inode_bitmap 正好占用 1 扇区,
    * 即 inode_bitmap_sects 等于 1, 所以位图中的位全都代表 inode_table 中的 inode,
    * 无须再像 block_bitmap 那样单独处理最后一扇区的剩余部分,
    * inode_bitmap 所在扇区中没有多余的无效位 */
...
static int search_file(const char* pathname, struct path_search_record* searched_record) {
    ...
    // 记录路径解析出来的各级名称, 如路径"/a/b/c", 则 name[] 为 a, b, c
    ...
    while (name[0]) {
        ...
        if (search_dir_entry(cur_part, parent_dir, name, &dir_e)) {
            memset(name, 0, MAX_FILE_NAME_LEN);
            // 若 sub_path 不等于NULL, 也就是未结束时继续拆分路径
            if (sub_path)
                sub_path = path_parse(sub_path, name);
...
int32_t sys_open(const char* pathname, uint8_t flags) {
   ...
   // 若是在最后一个路径上没找到, 并且并不是要创建文件, 直接返回 -1
    ...
    // 此fd是指任务 pcb->fd_table 数组中的元素下标, 并不是指全局 file_table 中的下标
    return fd;
}
...
// 删除文件(非目录), 成功返回 0, 失败返回 -1, 并不清除硬盘中的文件内容
...
static uint32_t get_parent_dir_inode_nr(uint32_t child_inode_nr, void* io_buf) {
    ...
    // 目录中的目录项".."中包括父目录inode编号,".."位于目录的第 0 块
...
// 把当前工作目录绝对路径写入 buf, size 是 buf的大小
...
char* sys_getcwd(char* buf, uint32_t size) {
   ...
   // 从下往上逐层找父目录,直到找到根目录为止.
    // 当 child_inode_nr 为根目录的 inode编号(0) 时停止
    // 即已经查看完根目录中的目录项
    ...
    // 至此 full_path_reverse中的路径是反着的, 即子目录在前(左),父目录在后(右) 
...
// 这里面是通过三层循环完成的, 最外层循环用来遍历通道, 中间层循环用来遍历通道中的硬盘
// 最内层循环用来遍历硬盘上的所有分区
void filesys_init(void) {
    // printk("super_block's size: 0x%x\n", sizeof(struct super_block));
    // printk("inode's size: 0x%x\n", sizeof(struct inode));
    // printk("dir_entry's size: 0x%x\n", sizeof(struct dir_entry));
    // printk("inode_tag's size: 0x%x\n", sizeof(struct list_elem));
