...
// 将 inode 写入到分区 part 中
// io_buf 是用于硬盘 io 的缓冲区, 必须由主调函数提前申请好
void inode_sync(struct partition* part, struct inode* inode, void* io_buf) {
    ...
    // 内存中 inode 中的成员 write_deny, inode_tag 和 i_open_cnts 在硬盘中是不需要的
...
struct inode* inode_open(struct partition* part, uint32_t inode_no) {
    // 先在 open_inodes 中找 inode, 此链表是为提速创建的缓冲区
    ...
    // 由于 open_inodes 链表中找不到, 从硬盘中读取, 先查找 inode_pos
    struct inode_position inode_pos;
    inode_locate(part, inode_no, &inode_pos);

    // 为了使 inode 置于内核空间被所有任务共享, 需要临时将当前任务 pcb->pgdir 置为 NULL
    // 待为 inode 完成内存分配后再将任务的 pgdir 恢复
    struct task_struct* cur = running_thread();
    uint32_t* cur_pagedir_bak = cur->pgdir;  // 备份当前进程的页目录
    cur->pgdir = NULL;
    inode_found = (struct inode*)sys_malloc(sizeof(struct inode));
    cur->pgdir = cur_pagedir_bak;   // 恢复 pgdir
    
    // inode_found 的内存分配好之后, 下面开始读取硬盘了, 考虑跨扇区的情况
    char* inode_buf;
    if (inode_pos.two_sec) {
    ...
    sys_free(inode_buf);    // 释放缓冲区 inode_buf
...
void inode_close(struct inode* inode) {
    ...
    if (--inode->i_open_cnts == 0) {
        ...
        // 需要释放 sys_malloc 为 inode 分配的内核空间
        struct task_struct* cur = running_thread();
...
void inode_release(struct partition* part, uint32_t inode_no) {
    ...
    // c inode 所有块地址已经收集到 all_blocks 中, 下面开始逐个回收
    block_idx = 0;
