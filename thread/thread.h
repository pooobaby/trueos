...
typedef void thread_func(void*);    // 自定义通用函数类型, 它将在很多线程函数中做为形参类型
...
初始情况下此栈在线程自己的内核栈中位置固定, 所在页的最顶端
*/
struct intr_stack {
    uint32_t vec_no;    // kernel.s 中 VECTOR 中 push %1压入的中断号
    uint32_t edi;
    uint32_t esi;
    uint32_t ebp;
...
struct thread_stack {
    ...
    void (*eip) (thread_func* func, void* func_arg);    // 函数指针成员
